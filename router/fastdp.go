...
func (fastdp *FastDatapath) getVxlanVportID(udpPort int) (odp.VportID, error) {
	...
	vxlanVportID, err := fastdp.dp.CreateVport(
		odp.NewVxlanVportSpec("vxlan", uint16(udpPort)))
	if err != nil {
		return 0, err
	}
...
func (fastdp *FastDatapath) VportCreated(ifindex int32, vport odp.Vport) error {
	fastdp.lock.Lock()
	defer fastdp.lock.Unlock()
...
func (fastdp *FastDatapath) VportDeleted(ifindex int32, vport odp.Vport) error {
	fastdp.lock.Lock()
	defer fastdp.lock.Unlock()
