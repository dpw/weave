...
// subsidiary overlays.  First, it passes a list of supported overalys
...
func (osw *OverlaySwitch) MakeForwarder(
	...
	for i, overlay := range overlays {
		...
		index := i
		params.SendControlMessage = func(msg []byte) error {
			xmsg := make([]byte, len(msg)+1)
			xmsg[0] = byte(index)
			copy(xmsg[1:], msg)
			return origSendControlMessage(xmsg)
		}
...
func (fwd *overlaySwitchForwarder) ControlMessage(msg []byte) {
	fwd.lock.Lock()
	subFwd := fwd.forwarders[msg[0]].fwd
	fwd.lock.Unlock()
	if subFwd != nil {
		subFwd.ControlMessage(msg[1:])
	}
}
